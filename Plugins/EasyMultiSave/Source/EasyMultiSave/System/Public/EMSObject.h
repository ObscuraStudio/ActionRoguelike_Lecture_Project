...
public:
	...
	bool SaveLevelActors(const bool bMemoryOnly, const TArray<TWeakObjectPtr<AActor>>& ActorListArray);
...
public:
	...
	void ProcessLevelActor(AActor* Actor, const FActorSaveData& ActorArray, const bool bSetupStructs);
...
private:
	...
	void InitGameActor(const FActorInitContext& Init);
...
public:
	...
	{
		return !RealLoadedActors.IsEmpty();
	}
...
public:
	...
	bool IsDifferentGameWorld(const UWorld* InWorld) const;
	...
	static bool IsRuntimeActor(const EActorType Type);
...
public:
	...
	{
		return !SavedActors.IsEmpty()
			|| !SavedScripts.IsEmpty()
			|| SavedGameMode.HasData()
			|| SavedGameState.HasData()
			|| MultiLevelStreamData.HasData();
	}

	inline const TArray<TWeakObjectPtr<AActor>> GetActorListAsArray() const
	{
		return ActorList.Array();
	}
...
public:
	...
	{
		return !bInitWorldPartitionLoadComplete && IsAsyncSaveOrLoadTaskActive(ESaveGameMode::MODE_Level, false);
	}
