...
void UDialogue::NPCFinishedTalking()
{
	...
	if (IsInitialized() && AvailableResponses.Num() && OwningComp && OwningComp->CurrentDialogue == this)
	{
		...
		if (OwningComp->HasAuthority())
		{
			...
			if (const UNarrativeDialogueSettings* DialogueSettings = GetDefault<UNarrativeDialogueSettings>())
			{
				if (AvailableResponses.Num() == 1)
				{
					if (DialogueSettings->bAutoSelectSingleResponse)
					{
						bWantsAutoSelect = true;
					}

					if (AvailableResponses[0] && AvailableResponses[0]->IsAutoSelectIfOnlyReply())
					{
						bWantsAutoSelect = true;
					}
				}
...
void UDialogue::PlayDialogueSound_Implementation(const FDialogueLine& Line, class AActor* Speaker, class AActor* Listener)
{
	...
	if (Line.DialogueSound)
	{
		...
		// Only bind to audio finished if GetLineDuration didn't provide a custom duration
		const float CustomDuration = GetLineDuration(CurrentNode, Line);
		const bool bHasCustomDuration = !FMath::IsNearlyEqual(CustomDuration, -1.f);

		if (DialogueAudio && Line.Duration == ELineDuration::LD_WhenAudioEnds && !bHasCustomDuration)
		{
