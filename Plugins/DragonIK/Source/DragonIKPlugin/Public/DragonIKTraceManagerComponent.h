...
*/
	...
	bool Hitmap_FindKeyExists(const TArray<FDragonIKTraceKeyValuePair>& array, FName key) const;
	FHitResult Hitmap_GetValueFromKey(const TArray<FDragonIKTraceKeyValuePair>& array, FName key) const;
	void Hitmap_SetValueOnKey(TArray<FDragonIKTraceKeyValuePair>& array, FName key, FHitResult value, FName bone_input);


	bool PendingTraces_FindKeyExists(const TArray<FDragonIKTraceParamKeyValuePair>& array, FName key) const;
	FDragonIKTraceMParams PendingTraces_GetValueFromKey(const TArray<FDragonIKTraceParamKeyValuePair>& array, FName key) const;
...
private:
	// The trace manager can be driven from animation worker threads via UDragonIK_Library::TraceSelectorFire,
	// while traces are executed on the game thread (TickComponent). These locks prevent data races.
	mutable FCriticalSection PendingCS;
	mutable FCriticalSection ResultsCS;

	// Fast lookup versions of the public arrays above.
	TMap<FName, FDragonIKTraceMParams> PendingMap;
	TMap<FName, FDragonIKTraceKeyValuePair> ResultsMap;
