...
public:	
	...
	// Calibration data computed synchronously on the game thread inside Grab_Weapon().
	// The animation node consumes these once on its first tick after a grab and then
	// marks itself calibrated without needing a multi-frame settling window.
	FTransform Calibrated_Weapon_Transform;
	FTransform Calibrated_Frozen_Weapon_Transform;
	FTransform Calibrated_Relative_Frozen_Weapon;
	FTransform Calibrated_Initial_IK_Bone_Transform;
	FTransform Calibrated_Frozen_RelMesh;
	FTransform Calibrated_Frozen_Pawn;
	bool Calibration_Data_Ready = false;

	// True from Grab_Weapon() through Release_Weapon(), independently of the
	// 0.2 s physics-handle delay that gates is_grabbing.
	bool weapon_ik_active = false;
