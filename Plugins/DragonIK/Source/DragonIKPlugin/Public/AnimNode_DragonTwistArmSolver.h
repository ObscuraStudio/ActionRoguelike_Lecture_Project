...
//		float ActualAlpha = 0;
	...
	FHitResult Line_Hit_Result;



	FTransform Hand_Transform_Saved;
	FTransform Shoulder_Transform_Saved;
...
//		FBoneSocketTarget LookAtTarget;
	...
	*/
	...
	/** Default the solver works in world space. This can be changed if needed. */
	UPROPERTY(EditAnywhere, Category=Settings)
	TEnumAsByte<enum EBoneControlSpace> EffectorLocationSpace = EBoneControlSpace::BCS_WorldSpace;

	/*
	 * This is only relevant if using bone space
	 */
	UPROPERTY(EditAnywhere, Category=Settings)
...
*/
	...
	*/
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = Settings, meta = (DisplayName = "Hand Rotation Method"))
...
//		FAxis LookAt_Neck_Axis;
	...
	*/
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = TraceCollisionSettings, meta = (DisplayName = "Use Trace Mode Logic ?", PinHiddenByDefault))
	bool TraceModeLogic= false;
...
* Trace channel used by the solver traces. Recommended to create a new dedicated trace channel for the ik through project settings.*/
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = TraceCollisionSettings, meta = (DisplayName = "Trace Channel",EditCondition="TraceModeLogic", PinHiddenByDefault))
	...
	* Choose Trace type - Line,Sphere and Box.*/
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = TraceCollisionSettings, meta = (DisplayName = "Trace Type",EditCondition="TraceModeLogic", PinHiddenByDefault))
	...
	* If trace type is box or sphere, its radius is controlled using the Trace Radius */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = TraceCollisionSettings, meta = (DisplayName = "Trace Radius",EditCondition="TraceModeLogic", PinHiddenByDefault))
	float Trace_Radius = 20;



	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = TraceCollisionSettings, meta = (DisplayName = "Trace Hit Complex ? (True for complex, False for simple)",EditCondition="TraceModeLogic", PinHiddenByDefault))
	bool line_trace_hit_complex = true;

	

	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = TraceCollisionSettings, meta = (DisplayName = "Trace Length Percentage",EditCondition="TraceModeLogic", PinHiddenByDefault))
	float Trace_Length_Percent = 120;





	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = TraceCollisionSettings, meta = (DisplayName = "Trace Firing Interval Constant (Performance)", EditCondition = "TraceModeLogic", PinHiddenByDefault))
	float trace_interval_duration = 0;


	float trace_timer_count = 1000;

	
	float Extra_Extension_Offset = 0;
...
//#endif
	...
	FTransform GetTargetTransform(const FTransform& InComponentTransform, FCSPose<FCompactPose>& MeshBases, FBoneSocketTarget& InTarget, EBoneControlSpace Space, const FTransform& InOffset);

	FTransform ConvertEffectorToWorldSpace(
...
//	virtual void EvaluateComponentSpace_AnyThread(FComponentSpacePoseContext& Output) override;
	...
	void line_trace_func(USkeletalMeshComponent& skelmesh, FVector startpoint, FVector endpoint, FHitResult RV_Ragdoll_Hit,  FName trace_tag, FHitResult& Output, FLinearColor debug_color);
