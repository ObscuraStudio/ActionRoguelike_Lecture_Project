...
*/
	...
	*/
	...
	// Cached vertical-only offset (along character up axis) from foot to terrain,
// captured at the moment the last trace fired. Used to re-anchor the hit point
// to the current foot position on non-trace frames.
	TArray<TArray<FVector>> feet_cs_vertical_offset_array;  // cached Impact_Original_Abs_Dir per foot
	TArray<TArray<FVector>> feet_last_impact_point_array;   // ImpactPoint used when gap was last computed
...
*/
	...
	/*
	* This only affects the knee bending. This can be usually ignored
	* This parameter is mainly useful when using mocap animations that involves the character regularly rotating its root bone in multiple directions.
