...
*/
	...
	/*
		...
		* Interpreted in the selected "Axis Input Space".
		*/
	...
	/*
	* Determines whether Forward/Up axis inputs are interpreted in Component Space (default, legacy behavior)
	* or World Space. World Space is useful when you want the axes to remain fixed in the world regardless
	* of character rotation.
	*/
	UPROPERTY(EditAnywhere, Category = LookAtSettings, meta = (DisplayName = "Axis Input Space", PinHiddenByDefault))
	EInputTransformSpace_DragonIK Axis_Input_Space = EInputTransformSpace_DragonIK::ENUM_ComponentSpaceSystem;
...
*/
	...
	// Resolved axis inputs used internally by the solver.
	// These are always in Component Space, regardless of Axis_Input_Space.
	FVector Resolved_LookAt_Axis_CS = FVector(0, 1, 0);
	FVector Resolved_Upward_Axis_CS = FVector(0, 0, 1);
	FVector Resolved_Reference_Forward_Axis_CS = FVector(0, 1, 0);
