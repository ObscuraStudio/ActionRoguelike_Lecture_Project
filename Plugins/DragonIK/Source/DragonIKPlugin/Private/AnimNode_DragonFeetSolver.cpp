...
void FAnimNode_DragonFeetSolver::UpdateInternal(const FAnimationUpdateContext& Context)
{
	...
	//if (enable_solver)
	{
		...
		// PREDICTIVE TRACE ORIGIN FIX: traces fire from last frame's bone positions
// but are consumed next frame. Shift all origins forward by horizontal velocity*dt
// to cancel the one-frame stale error. Vertical component excluded — the solver
// controls the up-axis itself.
		FVector Trace_Predictive_Offset = FVector::ZeroVector;
		if (Character_Actor != nullptr)
		{
			FVector CharVel = Character_Actor->GetVelocity();
			FVector UpDir = UKismetMathLibrary::TransformDirection(owner_skel_w_transform, character_direction_vector_CS);
			CharVel -= UpDir * FVector::DotProduct(CharVel, UpDir);
			Trace_Predictive_Offset = CharVel * delta_seconds_saved;
		}
				...
				if (i < spine_Feet_pair.Num() && i < spine_Transform_pairs.Num())
					for (int32 j = 0; j < spine_Feet_pair[i].Associated_Feet.Num(); j++)
					{
						...
						//	offseted_linetrace_location += (UKismetMathLibrary::TransformLocation(owner_skel_w_transform, dragon_input_data.FeetBones[spine_Feet_pair[i].order_index[j]].Feet_Trace_Offset));
							...
							FVector Front_point_location = spine_Transform_pairs[i].Associated_Feet[j].GetLocation() + Trace_Predictive_Offset + (character_forward_vector * dragon_input_data.FeetBones[spine_Feet_pair[i].order_index[j]].Front_Trace_Point_Spacing * scale_mode);
							...
							offseted_linetrace_location += Trace_Predictive_Offset;
							...
							*/
							...
							if(Affect_Toes_Always)
							{
								for (int32 k_finger = 0; k_finger < spine_Transform_pairs[i].Associated_Fingers[j].Num(); k_finger++)
								{
									...
									FVector finger_loc = owner_skel_w_transform.TransformPosition(feet_fingers_transform_array[i][j][k_finger].GetLocation() + trace_offset) + Trace_Predictive_Offset;



									FVector original_finger_loc = owner_skel_w_transform.TransformPosition(feet_fingers_transform_array[i][j][k_finger].GetLocation() + FVector(0, 0, trace_offset.Z))+ Trace_Predictive_Offset;
...

			{
				...
				if (spine_hit_pairs[spine_index].RV_Feet_Hits[feet_index].bBlockingHit)
				{
					...
					*/
					...
					// Cache Impact_Original_Abs_Dir and only recompute when ImpactPoint changes.
					// Between traces ImpactPoint is fixed but owner_skel_w_transform moves with
					// the character. InverseTransformLocation(moving_transform, fixed_point) gives
					// a different component-space value every frame. On small characters scale
					// amplifies this (divides by scale), making the oscillation visible as jitter.
					FVector CachedImpactAbsDir = FVector::ZeroVector;
					const FVector& CurrentIP = spine_hit_pairs[spine_index].RV_Feet_Hits[feet_index].ImpactPoint;
					const bool bCacheValid =
						spine_index < feet_last_impact_point_array.Num() &&
						feet_index < feet_last_impact_point_array[spine_index].Num() &&
						feet_last_impact_point_array[spine_index][feet_index].Equals(CurrentIP, 0.001f);
					if (bCacheValid)
					{
						CachedImpactAbsDir = feet_cs_vertical_offset_array[spine_index][feet_index];
					}
					else
					{
						Temp_Impact_Ref = UKismetMathLibrary::InverseTransformLocation(owner_skel_w_transform, CurrentIP);
						CachedImpactAbsDir = (Temp_Impact_Ref - EndBoneCSTransform.GetLocation()) * (Absolute_Up_Vector);
						if (spine_index < feet_cs_vertical_offset_array.Num() && feet_index < feet_cs_vertical_offset_array[spine_index].Num())
						{
							feet_cs_vertical_offset_array[spine_index][feet_index] = CachedImpactAbsDir;
							feet_last_impact_point_array[spine_index][feet_index] = CurrentIP;
						}
					}
					//Temp_Impact_Ref = EndBoneCSTransform.GetLocation();

					Temp_Impact_Ref = spine_hit_pairs[spine_index].RV_Feet_Hits[feet_index].ImpactPoint;
					Temp_Impact_Ref = UKismetMathLibrary::InverseTransformLocation(owner_skel_w_transform, Temp_Impact_Ref);


					FVector Impact_Original_Abs_Dir = CachedImpactAbsDir;
			...
			*/
			...
			if (solve_should_fail == false)
			{
				...
				if (test_counter < 10)
				{
					...
					feet_cs_vertical_offset_array.Empty();
					feet_last_impact_point_array.Empty();
				...
				if (test_counter < 10)
				{
					...
					feet_cs_vertical_offset_array.AddDefaulted(spine_Feet_pair.Num());
					feet_last_impact_point_array.AddDefaulted(spine_Feet_pair.Num());
				...
				for (int32 i = 0; i < spine_Feet_pair.Num(); i++)
				{
					...
					*/
					...
					if (test_counter < 10)
					{
						...
						feet_cs_vertical_offset_array[i].AddDefaulted(spine_Feet_pair[i].Associated_Feet.Num());
						feet_last_impact_point_array[i].AddDefaulted(spine_Feet_pair[i].Associated_Feet.Num());
