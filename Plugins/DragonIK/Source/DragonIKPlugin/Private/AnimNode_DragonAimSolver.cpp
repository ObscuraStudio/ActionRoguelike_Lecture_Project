...
static FVector DragonIK_ResolveAxisToComponentSpace(const FVector& AxisInput, EInputTransformSpace_DragonIK AxisInputSpace, const FTransform& MeshComponentTransform, const FVector& DefaultAxisCS)
{
	// Default/legacy behavior: axis inputs are interpreted in component space.
	FVector AxisCS = AxisInput;
	if (AxisInputSpace == EInputTransformSpace_DragonIK::ENUM_WorldSpaceSystem)
	{
		AxisCS = MeshComponentTransform.InverseTransformVectorNoScale(AxisInput);
	}

	AxisCS = AxisCS.GetSafeNormal();
	if (AxisCS.IsNearlyZero())
	{
		return DefaultAxisCS;
	}
	return AxisCS;
}
...
void FAnimNode_DragonAimSolver::EvaluateSkeletalControl_AnyThread(FComponentSpacePoseContext& Output, TArray<FBoneTransform>& OutBoneTransforms)
{
	if (toggle_alpha > 0.01f)
	{
		...
		// Resolve axis inputs into component space once per evaluation. All downstream math/libraries expect CS vectors.
		Resolved_LookAt_Axis_CS = DragonIK_ResolveAxisToComponentSpace(LookAt_Axis, Axis_Input_Space, owner_skel_w_transform, FVector(0, 1, 0));
		Resolved_Upward_Axis_CS = DragonIK_ResolveAxisToComponentSpace(Upward_Axis, Axis_Input_Space, owner_skel_w_transform, FVector(0, 0, 1));
		Resolved_Reference_Forward_Axis_CS = DragonIK_ResolveAxisToComponentSpace(Reference_Constant_Forward_Axis, Axis_Input_Space, owner_skel_w_transform, FVector(0, 1, 0));
...
void FAnimNode_DragonAimSolver::FABRIK_BodySystem(FCSPose<FCompactPose>& MeshBases, FComponentSpacePoseContext& Output, TArray<FBoneTransform>& OutBoneTransforms)
{
	...
	if (Use_Reference_Forward_Axis)
		Reference_Constant_Forward_Temp = Resolved_Reference_Forward_Axis_CS;
	else
		Reference_Constant_Forward_Temp = Resolved_LookAt_Axis_CS;
	...
	//	if (toggle_alpha > 0.01f)
	{
		...
		if (!enable_head_aim)
		{
			...
			LookAtLocation_Temp.SetLocation(Head_Orig_Transform.GetLocation() + Resolved_LookAt_Axis_CS * 100000);
		...
		if (enable_solver)
		{
			...
			LookatAxis_Temp.Axis = Resolved_LookAt_Axis_CS;
		...
		//	LookAtLocation_Temp = LookAtLocation_Saved;
			...
			*/
			...
			LookAtLocation_Temp.SetLocation(ClampRotateVector(LookAtLocation_Temp.GetLocation(), owner_skel_w_transform.TransformVector(Resolved_LookAt_Axis_CS), owner_skel_w_transform.TransformPosition(Head_Orig_Transform.GetLocation()), Verticle_Range_Angles.X, Verticle_Range_Angles.Y, Horizontal_Range_Angles.X, Horizontal_Range_Angles.Y, Use_Natural_Method));
							...
							if (Connector_Index.GetInt() > 0)
							{
								...
								if (!ignore_elbow_modification)
								{
									...
									if (reach_instead)
									{
										...
										Offseted_Hand_Transform.SetLocation(ClampRotateVector(owner_skel_w_transform.TransformPosition(Offseted_Hand_Transform.GetLocation()), owner_skel_w_transform.TransformVectorNoScale(Resolved_LookAt_Axis_CS), owner_skel_w_transform.TransformPosition(Shoulder_Transform_Output.GetLocation()), Aiming_Hand_Limbs[Main_Arm_Index].Max_Arm_V_Angle.X, Aiming_Hand_Limbs[Main_Arm_Index].Max_Arm_V_Angle.Y, Aiming_Hand_Limbs[Main_Arm_Index].Max_Arm_H_Angle.X, Aiming_Hand_Limbs[Main_Arm_Index].Max_Arm_H_Angle.Y, Use_Natural_Method));
									...
									arm_ik_params.LookAt_Axis_Input = Resolved_LookAt_Axis_CS;
									...
									arm_ik_params.Up_Vector_Val = Resolved_Upward_Axis_CS;
					...
					if ((Actual_Shoulder_Array[limb_index].IsValidToEvaluate() && Elbow_Array[limb_index].IsValidToEvaluate() && Hand_Array[limb_index].IsValidToEvaluate()) && (limb_index != Main_Arm_Index || Main_Arm_Index < 0 || reach_instead))
					{
						...
						if (Connector_Index.GetInt() > 0)
						{
							...
							if (Aiming_Hand_Limbs[limb_index].override_limits)
								Target_CS_Position = (ClampRotateVector(Arm_LookTarget, owner_skel_w_transform.TransformVectorNoScale(Resolved_LookAt_Axis_CS),  owner_skel_w_transform.TransformPosition(Head_Orig_Transform.GetLocation()), Aiming_Hand_Limbs[limb_index].Max_Arm_V_Angle.X, Aiming_Hand_Limbs[limb_index].Max_Arm_V_Angle.Y, Aiming_Hand_Limbs[limb_index].Max_Arm_H_Angle.X, Aiming_Hand_Limbs[limb_index].Max_Arm_H_Angle.Y, Use_Natural_Method));
							else
								Target_CS_Position = (ClampRotateVector(Arm_LookTarget, owner_skel_w_transform.TransformVectorNoScale(Resolved_LookAt_Axis_CS), owner_skel_w_transform.TransformPosition(Head_Orig_Transform.GetLocation()), Verticle_Range_Angles.X, Verticle_Range_Angles.Y, Horizontal_Range_Angles.X, Horizontal_Range_Angles.Y, Use_Natural_Method));
