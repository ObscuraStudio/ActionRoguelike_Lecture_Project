...
void FAnimNode_DragonSpineSolver::UpdateInternal(const FAnimationUpdateContext& Context)
{
	...
	/*
	for (int32 LinkIndex = 0; LinkIndex < AnimatedBoneTransforms.Num(); LinkIndex++)
	{
		...
		//if (CurrentLink.TransformIndex < AnimatedBoneTransforms.Num())
		{
			...
			FTransform WorldSpace_AnimBone = AnimatedBoneTransforms[LinkIndex].Transform;
			WorldSpace_AnimBone = WorldSpace_AnimBone * owning_skel->GetComponentTransform();

			UKismetSystemLibrary::DrawDebugSphere(owning_skel->GetWorld(), WorldSpace_AnimBone.GetLocation(), 10);
		}
	}
	*/
		...
		if (is_snake)
		{
			for (int i = 0; i < spine_hit_between.Num(); i++)
			{
				...
				line_trace_func(*Context.AnimInstanceProxy->GetSkelMeshComponent()
					, spine_between_transforms[i] + Trace_Predictive_Offset + character_direction_vector * start_const,
					spine_between_transforms[i] + Trace_Predictive_Offset - character_direction_vector * end_const,
					spine_hit_between[i],
		...
		for (int32 i = 0; i < spine_hit_pairs.Num(); i++)
		{
			if (spine_Feet_pair.Num() > i && (i == 0 || spine_Feet_pair[i].Associated_Feet.Num() > 0))
			{
				for (int32 j = 0; j < spine_Feet_pair[i].Associated_Feet.Num(); j++)
				{
					...
					//added recently to fix the movement vertical lag
					modified_feet_transform.AddToTranslation(Trace_Predictive_Offset); 
			...
			//	if(!feet_centre_point.ContainsNaN())
				{
					FVector feet_centre_point_pred = feet_centre_point + Trace_Predictive_Offset; // <-- added
					line_trace_func(*Context.AnimInstanceProxy->GetSkelMeshComponent(), feet_centre_point_pred + character_direction_vector * start_const, feet_centre_point_pred - character_direction_vector * end_const, spine_hit_pairs[i].Parent_Spine_Hit, spine_Feet_pair[i].Spine_Involved.BoneName, spine_Feet_pair[i].Spine_Involved.BoneName, spine_hit_pairs[i].Parent_Spine_Hit, FLinearColor::Red, false);
					...
					//if (Use_Fake_Chest_Rotations || Use_Fake_Pelvis_Rotations || bUse_Four_Cross_Lines)
					{
						...
						FVector feet_centre_point_xh = feet_centre_point + Trace_Predictive_Offset; // <-- added
						FVector spinal_left_upper = feet_centre_point_xh + character_direction_vector * (line_trace_upper_height * component_scale * virtual_scale ) + (right_dir * component_scale * virtual_scale * virtual_leg_width);
						FVector spinal_right_upper = feet_centre_point_xh + character_direction_vector * (line_trace_upper_height * component_scale * virtual_scale) - (right_dir * component_scale * virtual_scale * virtual_leg_width);
						FVector spinal_front_upper = feet_centre_point_xh + character_direction_vector * (line_trace_upper_height * component_scale * virtual_scale) + (forward_dir * component_scale * virtual_scale * virtual_leg_width);
						FVector spinal_back_upper = feet_centre_point_xh + character_direction_vector * (line_trace_upper_height * component_scale * virtual_scale) - (forward_dir * component_scale * virtual_scale * virtual_leg_width);


						line_trace_func(*Context.AnimInstanceProxy->GetSkelMeshComponent(), spinal_left_upper, feet_centre_point_xh - character_direction_vector * (line_trace_downward_height * component_scale * virtual_scale) + (right_dir * component_scale * virtual_scale * virtual_leg_width), spine_hit_pairs[i].Parent_Left_Hit, spine_Feet_pair[i].Spine_Involved.BoneName, spine_Feet_pair[i].Spine_Involved.BoneName, spine_hit_pairs[i].Parent_Left_Hit, FLinearColor::Green, true);
						line_trace_func(*Context.AnimInstanceProxy->GetSkelMeshComponent(), spinal_right_upper, feet_centre_point_xh - character_direction_vector * (line_trace_downward_height * component_scale * virtual_scale) - (right_dir * component_scale * virtual_scale * virtual_leg_width), spine_hit_pairs[i].Parent_Right_Hit, spine_Feet_pair[i].Spine_Involved.BoneName, spine_Feet_pair[i].Spine_Involved.BoneName, spine_hit_pairs[i].Parent_Right_Hit, FLinearColor::Green, true);

						line_trace_func(*Context.AnimInstanceProxy->GetSkelMeshComponent(), spinal_front_upper, feet_centre_point_xh - character_direction_vector * (line_trace_downward_height * component_scale * virtual_scale) + (forward_dir * component_scale * virtual_scale * virtual_leg_width), spine_hit_pairs[i].Parent_Front_Hit, spine_Feet_pair[i].Spine_Involved.BoneName, spine_Feet_pair[i].Spine_Involved.BoneName, spine_hit_pairs[i].Parent_Front_Hit, FLinearColor::Green, true);
						line_trace_func(*Context.AnimInstanceProxy->GetSkelMeshComponent(), spinal_back_upper, feet_centre_point_xh - character_direction_vector * (line_trace_downward_height * component_scale * virtual_scale) - (forward_dir * component_scale * virtual_scale * virtual_leg_width), spine_hit_pairs[i].Parent_Back_Hit, spine_Feet_pair[i].Spine_Involved.BoneName, spine_Feet_pair[i].Spine_Involved.BoneName, spine_hit_pairs[i].Parent_Back_Hit, FLinearColor::Green, true);
...
//			FRotator target_rot = FRotator(UKismetMathLibrary::Lerp<float>( spine_RotDifference[origin_point_index].Pitch, spine_RotDifference[origin_point_index].Pitch,0,adapt), final_rot.Yaw, spine_RotDifference[origin_point_index].Roll).GetNormalized();
		...
		//	GEngine->AddOnScreenDebugMessage(-1, 0.01f, FColor::Red, "target_rot Rot : " + (target_rot).ToString());
				...
				//if (LineTraceInitialized == true)
			{
				...
				if (Rotate_Around_Translate && complexity_type == ESolverComplexityPluginEnum::VE_Simple)
				{
					final_transformation.SetLocation(roll_based_location);
				}
...
DragonSpineOutput FAnimNode_DragonSpineSolver::DragonSpineProcessor_Direct(FTransform& EffectorTransform, FCSPose<FCompactPose>& MeshBases, TArray<FBoneTransform>& OutBoneTransforms)
{
	...
	
	{
		...
		if (complexity_type == ESolverComplexityPluginEnum::VE_Simple)
		{
		//	MaximumReach_Temp = Pelvic_Chest_distance; Removed to see how it feels
		}
