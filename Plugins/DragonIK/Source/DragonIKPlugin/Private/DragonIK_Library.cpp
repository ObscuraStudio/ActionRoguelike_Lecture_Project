...
void UDragonIK_Library::SetVectorComponentAlongDirection(FVector& VectorA, const FVector& VectorB, const FVector& Up_Vector)
{
	// Normalize the Up_Vector to ensure it's a unit vector
	FVector NormalizedUpVector = Up_Vector.GetSafeNormal();

	// Project VectorA and VectorB onto the Up_Vector using the dot product
	float ProjectionA = FVector::DotProduct(VectorA, NormalizedUpVector);
	float ProjectionB = FVector::DotProduct(VectorB, NormalizedUpVector);

	// Calculate the difference between the projections
	float Difference = ProjectionB - ProjectionA;

	// Adjust VectorA by adding the difference (scaled by the Up_Vector)
	VectorA += NormalizedUpVector * Difference;
}
...
void UDragonIK_Library::Evaluate_TwoBoneIK_Direct_Modified(FComponentSpacePoseContext& Output, USkeletalMeshComponent* owning_skel, ArmIKParamStruct arm_ik_params, FDragonData_ArmsData& HandData, float& last_shoulder_angle, TArray<FBoneTransform>& OutBoneTransforms)
{
	...
	if (HandData.Clavicle_Bone.IsValidToEvaluate() && arm_ik_params.Separate_Arms_Logic_Used)
	{
		...
		//FVector Shoulder_Trans = arm_ik_params.Shoulder_Trans.GetLocation();





		if (arm_ik_params.clavicle_only_bend_if_limit)
		{

			//	float reaching_ratio =   ((arm_ik_params.Shoulder_Trans.GetLocation() - arm_ik_params.ThighTransform.GetLocation()).Size()) - (arm_ik_params.Shoulder_Trans.GetLocation() - arm_ik_params.Hand_Trans.GetLocation()).Size();

			float reaching_ratio = ((arm_ik_params.Shoulder_Trans.GetLocation() - arm_ik_params.ThighTransform.GetLocation()).Size()) - ((arm_ik_params.Shoulder_Trans.GetLocation() - arm_ik_params.Knee_Trans.GetLocation()).Size() + (arm_ik_params.Knee_Trans.GetLocation() - arm_ik_params.Hand_Trans.GetLocation()).Size());

			float Clavicle_Size = (arm_ik_params.Shoulder_Trans.GetLocation() - clavicle_transform.GetLocation()).Size() * 2;
			//	Clavicle_Size = 15;
			reaching_ratio = FMath::Clamp(reaching_ratio, 0, Clavicle_Size) / Clavicle_Size;


			clavicle_rot_difference = clavicle_rot_difference * reaching_ratio;
...
//OLD1	JointTargetTransform.SetLocation(JointTargetTransform.GetLocation() + Pole_Transform.GetLocation());
...

	{
		...
		if (arm_ik_params.should_shoulder_twist_if_inward)
		{
			BS_UpperLimbTransform = SetArmYaw(HandData.invert_upper_twist, HandData.is_this_right_hand, last_shoulder_angle, arm_ik_params.mesh_component_transform, Output.Pose.GetComponentSpaceTransform(CachedUpperLimbIndex), Output.Pose.GetComponentSpaceTransform(CachedLowerLimbIndex), BS_UpperLimbTransform, BS_LowerLimbTransform);
		}
		else
		{
	...
	EDrawDebugTrace::Type DrawDebugType, FHitResult& OutHit, bool bIgnoreSelf, FLinearColor TraceColor, FName unique_identifier, FName bone_text)
{
	...
	trace_params.trace_complex = bTraceComplex;
...
void UDragonIK_Library::Evaluate_ConsecutiveBoneRotations(FComponentSpacePoseContext& Output, USkeletalMeshComponent* owning_skel, FTransform mesh_component_transform, TArray<FDragonData_SpineFeetPair> spine_feet_pair, FRuntimeFloatCurve Look_Bending_Curve, FBoneReference RootBone_Input, FBoneReference TipBone_Input, float Lookat_Radius, FRotator Inner_Body_Clamp, FTransform EffectorTransform, FAxis LookAt_Axis, float Lookat_Clamp, float Vertical_Dip_Treshold, float Downward_Dip_Multiplier, float Inverted_Dip_Multiplier, float Side_Multiplier, float Side_Down_Multiplier, bool alter_pelvis, FTransform PelvisLocTarget, FRuntimeFloatCurve Bending_Multiplier_Curve, float Up_Rot_Clamp, bool use_natural_rotation, bool separate_head_clamp, float head_clamp_value, FTransform HeadTransf, bool head_rot_override, EInputTransformSpace_DragonIK transform_space, TArray<FName> Bone_Input_Skeleton, TArray<FBoneTransform>& OutBoneTransforms)
{
	...
	if (Bone_Input_Skeleton.Num() > 1)
	{
		RootBone = FBoneReference(Bone_Input_Skeleton[0]);
	}
	...
	if (Bone_Input_Skeleton.Num() > 1)
	{
		TipBone = FBoneReference(Bone_Input_Skeleton[Bone_Input_Skeleton.Num() - 1]);
	}
	...
	if (Bone_Input_Skeleton.Num() > 1)
	{
		...
		for (int bone_i = 0; bone_i < Bone_Input_Skeleton.Num(); bone_i++)
		{
			//	int inverted_index = Bone_Input_Skeleton.Num()-bone_i-1;

			FBoneReference BoneRef = FBoneReference(Bone_Input_Skeleton[bone_i]);

			BoneRef = FBoneReference(Bone_Input_Skeleton[bone_i]);
...
void UDragonIK_Library::RectifyNaNRotator(FRotator& Rotator)
{
	// Check each component of the rotator for NaN and fix it if necessary
	if (FMath::IsNaN(Rotator.Pitch))
	{
		//	UE_LOG(LogTemp, Warning, TEXT("Pitch is NaN, resetting to 0"));
		Rotator.Pitch = 0.0f;
	}

	if (FMath::IsNaN(Rotator.Yaw))
	{
		//	UE_LOG(LogTemp, Warning, TEXT("Yaw is NaN, resetting to 0"));
		Rotator.Yaw = 0.0f;
	}

	if (FMath::IsNaN(Rotator.Roll))
	{
		//	UE_LOG(LogTemp, Warning, TEXT("Roll is NaN, resetting to 0"));
		Rotator.Roll = 0.0f;
	}
}
