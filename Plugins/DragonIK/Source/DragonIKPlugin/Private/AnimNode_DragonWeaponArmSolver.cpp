		...
		if (Internal_Switched_Interpolation_bool)
		{
			if (weapon_handler_component != nullptr)
			{
					...
					for (int alpha_index = 0; alpha_index < grab_alpha.Num(); alpha_index++)
					{
						...
						if (weapon_handler_component->arm_reset_queue_index.Num() > 0 && alpha_index < weapon_handler_component->arm_reset_queue_index.Num())
							if (weapon_handler_component->arm_reset_queue_index[alpha_index] == true)
							{
								...
								if (grab_alpha[alpha_index] == 0 && Calculated_Hand_Transforms.Num() > 0 && weapon_handler_component->weapon_ik_active && weapon_handler_component->weapon_input_component)
								{
									...
									FTransform weapon_transform = weapon_handler_component->weapon_input_component->GetComponentTransform();

									if (USkeletalMeshComponent* Casted_SkeletalMeshComponent = Cast<USkeletalMeshComponent>(weapon_handler_component->weapon_input_component))
									{
				...
				for (int status_index = 0; status_index < Calibration_Status_Array.Num(); status_index++)
				{
					if (weapon_handler_component != nullptr)
					{
						if (!weapon_handler_component->weapon_ik_active)
						{
				...
				if (owning_skel->GetBoneIndex(wep_grabbed_bone_selected) > -1 && weapon_handler_component->weapon_ik_active && weapon_handler_component->weapon_input_component)
				{
			...
			if (Calculated_Hand_Transforms.Num() > 0 && owning_skel != nullptr)
			{
				...
				for (int arm_index = 0; arm_index < Arm_TargetLocation_Overrides.Arm_TargetLocation_Overrides.Num(); arm_index++)
				{
					...
					if ((Calculated_Hand_Transforms.Num() > arm_index && Hand_Array.Num() > arm_index) && weapon_handler_component != nullptr)
					{
						...
						if (weapon_handler_component->weapon_ik_active && weapon_handler_component->weapon_input_component != nullptr)
						{

							FTransform Weapon_World_Transform = weapon_handler_component->weapon_input_component->GetComponentTransform();
							...
							//if (weapon_handler_component->perform_grab_weapon)
							{
								...
								if (Calibration_Status_Array[arm_index] == false && weapon_handler_component->Calibration_Data_Ready)
								{
									// Consume the game-thread calibration snapshot captured in Grab_Weapon().
									// Both the weapon and bone transforms were read at the same game-thread
									// moment so they are internally consistent regardless of animation speed.
									Frozen_Weapon_Transform = weapon_handler_component->Calibrated_Frozen_Weapon_Transform;
									Relative_Frozen_Weapon_Transform = weapon_handler_component->Calibrated_Relative_Frozen_Weapon;
									Initial_IK_Hand_Transform = weapon_handler_component->Calibrated_Initial_IK_Bone_Transform;
									Frozen_RelMesh_Transform = weapon_handler_component->Calibrated_Frozen_RelMesh;
									Frozen_Pawn_Transform = weapon_handler_component->Calibrated_Frozen_Pawn;

									Frozen_Hand_Transform = MeshBases.GetComponentSpaceTransform(Hand_Array[primary_hand_index].CachedCompactPoseIndex);

									Hand_Weapon_Relative_Transforms[arm_index] = Calculated_Hand_Transforms[arm_index]
										* weapon_handler_component->Calibrated_Weapon_Transform.Inverse();

									Calibration_Status_Array[arm_index] = true;

									// Clear the shared flag after the last arm has consumed it.
									if (arm_index == Aiming_Hand_Limbs.Num() - 1)
										weapon_handler_component->Calibration_Data_Ready = false;
								}
