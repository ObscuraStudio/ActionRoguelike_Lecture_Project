...
void UDragonIKWeaponPhysicsComponent::SetWeaponPhysicsBlendWeight(float InWeight)
{
	...
	FTransform PhysicsTransform = weapon_input_component->GetBodyInstance()->GetUnrealWorldTransform();
	...
	weapon_input_component->SetRelativeTransform(BlendedTransform);
}
...
void UDragonIKWeaponPhysicsComponent::ModifyWeaponTransform()
{
	...
	if (is_grabbing && weapon_physics_handle != nullptr && owning_skeleton != nullptr)
	{
		...
		if (weapon_input_component != nullptr)
		{
			if ((Weapon_Delta_Transform.GetLocation() - weapon_input_component->GetComponentTransform().GetLocation()).Length() > Teleport_Distance)
			{
			...
			if (teleport_freeze_count < 10 && teleport_freeze_count > 0)
			{
				...
				weapon_input_component->SetPhysicsLinearVelocity(FVector::ZeroVector, false);
				weapon_input_component->SetPhysicsAngularVelocityInRadians(FVector::ZeroVector, false);
				weapon_input_component->SetWorldTransform(Saved_Relative_Hand_Transform * owning_skeleton->GetSocketTransform(weapon_grabbed_bone, ERelativeTransformSpace::RTS_World), false, nullptr, ETeleportType::TeleportPhysics);
		...
		Weapon_Delta_Transform = weapon_input_component->GetComponentTransform();
...
void UDragonIKWeaponPhysicsComponent::DelayedGrabEvent()
{
	...
	if (owning_skeleton->GetBoneIndex(weapon_grabbed_bone) > -1)
	{
		...
		//last_saved_hand_transform = weapon_input_component->GetComponentTransform();


		//weapon_physics_handle->GrabComponentAtLocationWithRotation(active_weapon_component, "", weapon_input_component->GetComponentLocation(), weapon_input_component->GetComponentRotation());
		...
		*/
		...
		Saved_Relative_Hand_Transform = weapon_input_component->GetComponentTransform();
		...
		//last_saved_weapon_transform =  weapon_input_component->GetComponentTransform()* dominant_hand_transform.Inverse();
		last_saved_weapon_transform = weapon_input_component->GetComponentTransform();
		...
		if (!last_saved_hand_transform.Equals(FTransform::Identity))
		{
			...
			{
				weapon_input_component->SetCollisionEnabled(ECollisionEnabled::QueryAndPhysics);
				weapon_input_component->SetSimulatePhysics(true);
			}
			else
			{
				weapon_input_component->SetSimulatePhysics(false);
			}


			weapon_input_component->SetEnableGravity(perform_gravity_simulation);
			...
			//if (physics_welded_weapon == false)
			{
				...
				{
					weapon_physics_handle->GrabComponentAtLocationWithRotation(weapon_input_component, weapon_bone, last_saved_hand_transform.GetLocation(), last_saved_hand_transform.Rotator());
			...
			//weapon_physics_constraint->SetWorldLocation(weapon_input_component->GetComponentTransform().GetLocation());
			...
			FVector const cached_active_vector = weapon_input_component->GetComponentLocation();
			//weapon_input_component->SetWorldLocation(dominant_shoulder.GetLocation()+ (weapon_input_component->GetComponentLocation() - dominant_shoulder.GetLocation()).GetUnsafeNormal()*100);


			//FTransform weapon_hand_offset =  (weapon_input_component->GetComponentTransform() *  owning_skeleton->GetBoneTransform(owning_skeleton->GetBoneIndex(weapon_grabbed_bone)).Inverse());

			FVector weapon_hand_offset = (weapon_input_component->GetComponentLocation() - dominant_hand_transform.GetLocation());

			//  COMMENTED FOR NOW
			//	weapon_input_component->SetWorldLocation( (dominant_shoulder.GetLocation() + weapon_hand_offset));
			weapon_input_component->SetWorldLocation((dominant_shoulder.GetLocation()), false, nullptr, ETeleportType::TeleportPhysics);

			if (UDragonIK_Library::IsBonePhysicsEnabled(owning_skeleton, dominant_shoulder_name))
			{
				weapon_physics_constraint->SetConstrainedComponents(owning_skeleton, dominant_shoulder_name, weapon_input_component, "None");
			}
			else
			{
				...
				for (int i = 0; i < 5; i++)
				{
					...
					{
						weapon_physics_constraint->SetConstrainedComponents(owning_skeleton, shoulder_temp_bone, weapon_input_component, "None");
			...
			weapon_input_component->SetWorldLocation(cached_active_vector, false, nullptr, ETeleportType::TeleportPhysics);



			float weapon_shoulder_length = (dominant_shoulder.GetLocation() - weapon_input_component->GetComponentLocation()).Length();
		...
		else
		{
			...
			//weapon_input_component->AttachToComponent(owning_skeleton,FAttachmentTransformRules::KeepWorldTransform,weapon_grabbed_bone);
			...
			//weapon_input_component->SetWorldLocation(temp_hand_transform.GetLocation());
...
void UDragonIKWeaponPhysicsComponent::DisableWeaponsArms(bool reset_weapon_transform = true)
{
	...
	if (weapon_input_component)
	{
		...
		if (reset_weapon_transform)
			weapon_input_component->SetWorldTransform(Saved_Relative_Hand_Transform * dominant_hand_transform);
...
void UDragonIKWeaponPhysicsComponent::EnableWeaponsArms()
{
	...
	if (weapon_input_component)
	{
		...
		weapon_input_component->DetachFromComponent(FDetachmentTransformRules::KeepWorldTransform);
...
void UDragonIKWeaponPhysicsComponent::Release_Weapon(bool should_simulatephysics, bool use_gravity, bool bKeep_Attachment)
{
	...
	if (weapon_input_component != nullptr)
	{
		weapon_input_component->SetEnableGravity(use_gravity);
		weapon_input_component->SetSimulatePhysics(should_simulatephysics);
	}
	...
	if (bKeep_Attachment && weapon_input_component != nullptr)
	{
...
void UDragonIKWeaponPhysicsComponent::Grab_Weapon(FName grabbed_bone, bool simulate_physics, bool use_gravity, FName WeaponBoneName)
{
	if (owning_skeleton != nullptr)
	{
		...
		if (weapon_input_component != nullptr && owning_skeleton->GetBoneIndex(grabbed_bone) > -1)
		{
			...
			weapon_input_component->SetIsReplicated(true);
			...
			//if(perform_physics_simulation)
			//weapon_input_component->AttachToComponent(owning_skeleton, FAttachmentTransformRules::FAttachmentTransformRules(EAttachmentRule::KeepWorld, EAttachmentRule::KeepWorld, EAttachmentRule::KeepWorld, false), weapon_grabbed_bone);
			...
			weapon_input_component->SetEnableGravity(use_gravity);
			last_saved_relative_wep_transform = weapon_input_component->GetRelativeTransform();

			// Compute calibration data synchronously on the game thread so the
			// animation node can consume it instantly on its next tick without
			// needing a multi-frame settling window.
			FTransform skel_world = owning_skeleton->GetComponentTransform();

			Calibrated_Weapon_Transform = weapon_input_component->GetComponentTransform();
			if (USkeletalMeshComponent* CastedSkelMesh = Cast<USkeletalMeshComponent>(weapon_input_component))
			{
				int calib_bone_idx = CastedSkelMesh->GetBoneIndex(WeaponBoneName);
				if (calib_bone_idx > -1)
					Calibrated_Weapon_Transform = CastedSkelMesh->GetBoneTransform(calib_bone_idx);
			}
			Calibrated_Weapon_Transform.SetScale3D(FVector::OneVector);

			Calibrated_Frozen_Weapon_Transform = Calibrated_Weapon_Transform;
			Calibrated_Relative_Frozen_Weapon = Calibrated_Weapon_Transform * skel_world.Inverse();

			int calib_grab_bone_idx = owning_skeleton->GetBoneIndex(grabbed_bone);
			Calibrated_Initial_IK_Bone_Transform = owning_skeleton->GetBoneTransform(calib_grab_bone_idx) * skel_world.Inverse();

			Calibrated_Frozen_RelMesh = owning_skeleton->GetRelativeTransform();
			if (owning_skeleton->GetOwner() != nullptr)
				Calibrated_Frozen_Pawn = owning_skeleton->GetOwner()->GetActorTransform();

			Calibration_Data_Ready = true;
			weapon_ik_active = true;
...
void UDragonIKWeaponPhysicsComponent::Destroy_Weapon(AActor* weapon)
{
	...
	if (weapon_input_component != nullptr)
	{
		weapon_input_component->GetOwner()->Destroy();
		UKismetSystemLibrary::PrintText(owning_skeleton->GetOwner()->GetWorld(), FText::AsCultureInvariant(" DESTROY TEST!"), true, false, FLinearColor::Yellow, 10.1f);
