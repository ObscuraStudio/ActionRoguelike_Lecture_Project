...
void FDragonAimSolverEditMode::Render(const FSceneView* View, FViewport* Viewport, FPrimitiveDrawInterface* PDI)
{
	...
	if (RuntimeNode->DisplayLineTrace)
	{
		...
		{
			const float Scale = View->WorldToScreen(StartTransform.GetLocation()).W * (4.0f / View->UnscaledViewRect.Width() / View->ViewMatrices.GetViewToClip().M[0][0]);
		...
		for (int32 i = 0; i < RuntimeNode->dragon_input_data.FeetBones.Num(); i++)
		{
			...
			if (RuntimeNode->knee_Animated_transform_array.Num() > i)
			{
				...
				const float Scale = View->WorldToScreen(StartTransformKnee.GetLocation()).W * (4.0f / View->UnscaledViewRect.Width() / View->ViewMatrices.GetViewToClip().M[0][0]);
		...
		for (int32 i = 0; i < RuntimeNode->Aiming_Hand_Limbs.Num(); i++)
		{
			...
			if (RuntimeNode->Use_Separate_Targets)
			{
				...
				if (RuntimeNode->Debug_Hand_Locations.Num() > i)
				{
					...
					const float HandScale = View->WorldToScreen(Hand_Default_Transform.GetLocation()).W * (4.0f / View->UnscaledViewRect.Width() / View->ViewMatrices.GetViewToClip().M[0][0]);
			...
			if (RuntimeNode->reach_instead)
			{
				...
				if (RuntimeNode->nsew_pole_method)
				{
					...
					if (RuntimeNode->Elbow_Bone_Transform_Array.Num() > i)
					{
						...
						const float NorthScale = View->WorldToScreen(NorthPoleTransform.GetLocation()).W * (4.0f / View->UnscaledViewRect.Width() / View->ViewMatrices.GetViewToClip().M[0][0]);
						...
						const float SouthScale = View->WorldToScreen(SouthPoleTransform.GetLocation()).W * (4.0f / View->UnscaledViewRect.Width() / View->ViewMatrices.GetViewToClip().M[0][0]);
						...
						const float EastScale = View->WorldToScreen(EastPoleTransform.GetLocation()).W * (4.0f / View->UnscaledViewRect.Width() / View->ViewMatrices.GetViewToClip().M[0][0]);
						...
						const float WestScale = View->WorldToScreen(WestPoleTransform.GetLocation()).W * (4.0f / View->UnscaledViewRect.Width() / View->ViewMatrices.GetViewToClip().M[0][0]);
